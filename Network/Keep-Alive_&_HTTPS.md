...
## 5 HTTPS 개요
> HTTPS는 평문(text)을 주고 받는 HTTP에  
> TLS로 메시지를 암호화하는 것을 추가한 프로토콜이다.

### 5.1 HTTPS의 필요성
HTTP는 네트워크의 중간에서 누구든 패킷을 가로채서 내용을 볼 수 있다.
→ HTTP에서 통신 내용을 안전하게 전달하기 위한 방법이 필요
...
2. **서버 신뢰성**  
   HTTP → 통신하는 서버가 진짜 서버인지 검증 불가  
   HTTPS → 인증서를 통해 서버 신원 검증
...
4. **성능 특성**  
   HTTP → 연결만 성립하면 바로 통신  
   HTTPS → TLS handshake 비용 발생

### 5.3 TLS/SSL 개념 정리
- **SSL vs TLS**  
  SSL은 옛날 이름이다.  
  현재는 **TLS(Transport Layer Security)** 가 정식 용어이다.
...
- **TLS의 역할 3가지**  
  ...
  2. **무결성 (Integrity)**  
     ...
     **변조 방지**  
     → MAC(Message Authentication Code)로 한 비트라도 바뀌면 즉시 탐지
...
## 6 TLS Handshake
> 서로 처음 만난 두 통신 주체가  
> **신원을 확인하고 안전하게 쓸 공통 비밀키를 합의**하는 과정

### 6.1 TLS handshake 전체 흐름
1. **ClientHello**  
   클라이언트 → 서버  
   
   아래의 정보를 서버로 보낸다.
   - 지원하는 TLS 버전
   - 지원하는 암호 스위트 목록 (Cipher Suite는 TLS 통신에서 사용할 암호 알고리즘들의 조합이다.)
   - 클라이언트 랜덤 값
   - SNI (하나의 서버에서 여러 도메인과 여러 인증서를 사용할 경우 어떤 도메인인지 알려주기 위한 메커니즘)
   - 클라이언트 Key Share (ECDHE 공개키)

   ⇒ 서버로 가능한 방식과 키 교환 정보를 전달한다.
...
3. **인증서 검증**
   ...
   **인증서 체인**은 서버 인증서부터 중간 CA, 루트 CA까지 누가 누구를 서명했는지에 따라 연결된 구조이며,  
   클라이언트는 서버가 제공한 인증서 체인의 전자서명을 차례로 검증하고,  
   최종적으로 루트 CA가 자신이 신뢰하는 CA 저장소에 있는지를 기준으로 서버 인증서의 신뢰 여부를 판단한다.  

   → 이 과정으로 **서버가 보낸 공개키도 진짜 그 도메인의 공개키임이 보장**된다.

4. **대칭 키 생성**  
   ECDHE 방식을 통해 클라이언트와 서버는  
   각자 계산으로 동일한 공유 비밀(shared secret)을 만들고,  
   이 값으로 대칭키를 파생한다.  
   
   그리고 이후의 데이터 교환은 그 대칭키를 통해 암호화/복호화해서 통신한다.  
   (인증서는 이 과정에서 정보 교환을 정상 서버와 했다는 것을 보장하기 위한 것이다.)

### 6.2 TLS handshake가 비싼 이유
1. **RTT를 여러 번 사용한다.**
   
   TLS handshake는 TCP 3-way handshake 이후에 **추가적인 왕복(RTT)** 을 필요로 한다.

2. **비대칭 암호 연산 비용**

   암호화를 위해 적지 않은 CPU 연산 비용 발생

3. **인증서 검증 비용**

   체인 검증, 서명 검증, 폐기 목록 확인 등
   
## 7 HTTPS와 Keep-Alive의 관계
> HTTPS에서 Keep-Alive는 "옵션"이 아니라  
> 성능과 안정성을 좌우하는 "전제 조건"에 가깝다.

### 7.1 HTTPS에서 Keep-Alive가 더 중요한 이유
HTTP는 TCP handshake 비용만 있지만,  
HTTPS는 TCP handshake에 TLS handshake 비용이 추가로 발생한다.

### 7.2 TLS handshake 비용 vs TCP handshake 비용
- **TCP handshake**  
  주로 RTT.  
  네트워크 지연에 민감.
...
- **TLS handshake**  
  ...
  → 대규모 트래픽에서는 TLS handshake를 줄이는 것이 중요한다.
